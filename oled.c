...
#define SPI_CMD_PATH	"/sys/devices/platform/soc/ffd00000.cbus/ffd13000.spi/spi_master/spi0/spi0.0/graphics/fb0/cmd"
#define SPI_DATA_PATH	"/sys/devices/platform/soc/ffd00000.cbus/ffd13000.spi/spi_master/spi0/spi0.0/graphics/fb0/data"
...
int write_cmd(uint8_t cmd)
{
	...
	fd = open(SPI_CMD_PATH, O_RDWR);
	if(fd < 0) {
		printf("can't open spi cmd path %d\n", fd);
		return -1;
	}

	nwr = sprintf(value, "%d", cmd);
	ret = write(fd, value, nwr); 

	if (ret != nwr) {
		printf("cmd write fail %d\n", ret);
		close(fd);
		return -1;
	}
...
int write_data(uint8_t data)
{
	...
	fd = open(SPI_DATA_PATH, O_RDWR);
	if(fd < 0) {
		printf("can't open spi data path %d\n", fd);
		return -1;
	}

	nwr = sprintf(value, "%2d", data);
	ret = write(fd, value, nwr); 

	if (ret != nwr) {
		printf("data write fail %d\n", ret);
		close(fd);
		return -1;
	}
...
{
	Write_Command(0x20|d );			// Set Memory Addressing Mode
									// Default => 0x20
									// 0x20 => Horizontal Addressing Mode
									// 0x21 => Vertical Addressing Mode						
}
...
void Set_Segment_Remap(unsigned char d)
{
	Write_Command(d);				// Set Segment Re-Map
									...
									// 0xA1 => Column Address 0 Mapped to SEG132		   
}
...
void Set_Charge_Pump(unsigned char a)
{
	Write_Command(0xAD);
	Write_Command(0x8A|a);			// 	Set DC-DC enable (a=0:disable; a=1:enable) 
...
//  Patterns
//=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=-=
unsigned char UniV[][48]={
0X00,0X00,0X1E,0XFE,0XFE,0XE0,0X00,0X00,0X80,0XF0,0XF8,0XF0,0X80,0X00,0X00,0XE0,
0XFE,0XFE,0X1E,0X00,0X00,0X78,0XFC,0XFE,0XFE,0XCE,0XCE,0XCE,0XCE,0XCE,0XCE,0XCE,
0X8E,0X06,0X00,0X00,0X00,0X0E,0X0E,0X0E,0XFE,0XFE,0XFE,0X0E,0X0E,0X0E,0X00,0X00,
0X00,0X00,0X00,0X01,0X0F,0X3F,0X3C,0X3E,0X0F,0X07,0X03,0X07,0X0F,0X3E,0X3C,0X3F,
0X0F,0X01,0X00,0X00,0X00,0X30,0X38,0X39,0X39,0X39,0X39,0X39,0X39,0X39,0X3F,0X3F,
0X3F,0X1F,0X00,0X00,0X00,0X38,0X38,0X38,0X3F,0X3F,0X3F,0X38,0X38,0X38,0X00,0X00,
...
unsigned char Ascii_1[240][5]={		// Refer to "Times New Roman" Font Database...
	...
	{0x44,0x28,0x10,0x28,0x44},		//   ( 88)  x - 0x0078 Latin Small Letter 
...
void Off_pattern(void)//??
{
	...
	for(i=0;i<16;i++)
	{
  		...
  		Write_Command(0xb0+i);//set high column address
...
void OLED_Init_E()				// VCC Generated by External
{
	if (!enable_oled)
		return;

	Set_Display_On_Off(0xAE);		    // Display Off (0xAE/0xAF)
	Set_Multiplex_Ratio(0x7F);			// A8H: 1/128 Duty (0x0F~0x3F)
	Set_Display_Clock(0x50);		    // D5H; Set Clock as 104 Frames/Sec                         
	Set_Common_Remap(0xC0);				// Set  C OM/Row Scan Direction (0xC0/0xC8)		 
	Set_Display_Offset(0x00);			// D3H: Shift Mapping RAM Counter (0x00~0x3F)       
	Set_Start_Line(0x00);				// DCH: Set Mapping RAM Display Start Line (0x00~0x3F)	    
	Set_Addressing_Mode(0);			    // Set Memory addressing mode (0x20/0x21)				
	Set_Contrast_Control(Brightness);	// 81H: Set SEG Output Current                                
	Set_Segment_Remap(0xA0);			// Set SEG/Column Mapping (0xA0/0xA1)                  
	Set_Entire_Display(0xA4);			// Disable Entire Display On (0xA4/0xA5)   		 
	Set_Inverse_Display(0xA6);			// Disable Inverse Display On (0xA6/0xA7)  
	Set_Charge_Pump(0);					// ADH: Set Built-in DC/DC Converter 	 	 
	Set_Precharge_Period(0x22);			// D9H ;Set Pre-Charge as 2 Clocks & Discharge as 2 Clocks    
	Set_VCOMH(0x35);					// DBH;  Set COM Deselect Level 	  
	Fill_RAM(0x00);						// Clear Screen
	Set_Display_On_Off(0xAF);			// Display On (0xAE/0xAF)

}
#if 0
void OLED_Display(int wifi, int server, int sensor)
{
	time_t ct;
	time_t t = time(NULL);
	struct tm tm = *localtime(&t);

	unsigned char IOT_BOX[]={96, 96, 41, 47, 52, 108, 34, 47, 56, 0};
	unsigned char Wifi_OK[]={55, 41, 38, 41, 96, 96, 96, 26, 96, 49,43, 96, 96, 0};
	unsigned char Wifi_FAIL[]={55, 41, 38, 41, 96, 96, 96, 26, 96, 38, 33, 41, 44, 0};
	unsigned char Server_OK[]={51, 37, 50, 54, 37, 50, 96, 26, 96, 49, 43, 96, 96, 0};
	unsigned char Server_FAIL[]={51, 37, 50, 54, 37, 50, 96, 26, 96, 38, 33, 41, 44, 0};
	unsigned char SENSOR_OK[]={51, 37, 46, 51, 47, 50, 96, 26, 96, 49, 43,96, 96, 0};
	unsigned char SENSOR_FAIL[]={51, 37, 46, 51, 47, 50, 96, 26, 96, 38, 33, 41, 44, 0};
	unsigned char time[]={96, 96, 96, 96, 96, 96, 96, 96, 16, 16, 26, 16, 16, 26, 16, 16, 0};
/*
	if (wifi) {
		Set_Led(WIFI_LED1, 1);
		Set_Led(WIFI_LED2, 0);
	...
	if (server) {
		Set_Led(SERVER_LED1, 1);
		Set_Led(SERVER_LED2, 0);
	} else {
		Set_Led(SERVER_LED1, 0);
		Set_Led(SERVER_LED2, 1);
	}
*/
	if (!enable_oled)
		return 0;
	
	time[8] += (tm.tm_hour / 10);
	time[9] += (tm.tm_hour % 10);
	time[11] += (tm.tm_min / 10);
	time[12] += (tm.tm_min % 10);
	time[14] += (tm.tm_sec / 10);
	time[15] += (tm.tm_sec % 10);
	Show_String(1,&time,0x01,XLevel+0x20);

	Show_String(1,&IOT_BOX,0x04,XLevel+0x20);

	if (wifi)
		Show_String(1,&Wifi_OK,0x08,XLevel+0x20);
	else
		Show_String(1,&Wifi_FAIL,0x08,XLevel+0x20);
	
	if (server)
		Show_String(1,&Server_OK,0x0A,XLevel+0x20);
	else
		Show_String(1,&Server_FAIL,0x0A,XLevel+0x20);

	if (sensor)
		Show_String(1,&SENSOR_OK,0x0C,XLevel+0x20);
	else
		Show_String(1,&SENSOR_FAIL,0x0C,XLevel+0x20);
}
#endif
void OLED_PowerOff(void)
{
	unsigned char string[]={96, 96, 96, 96, 96, 96, 96, 96, 47, 38, 38, 0};

	if (!enable_oled)
		return;
	
	Show_String(1, string, 0x0C, XLevel+0x22);
}
...
{
	int ret = 0;
	//unsigned char Name[]={55,41,51,37,35,40,41,48,0};
	//					// WiseChip
	unsigned char Name[]={45,54,52,37,35,40,0};
						// MVTECH
	unsigned char Tel_1[]={11,24,24,22,13,19,23,0};
						// professional
	unsigned char Tel_2[]={96,96,96,96,21,24,23,17,22,24,0};

	OLED_Init_E();

	while(1)
	{
		// Show Pattern - WSI OLED
		printf("Show Pattern\n");
		Show_Pattern(&UniV[0][0],0x07,0x0A,XLevel+0x25,0x30);
		sleep(2);                  	
		...
		// Show String 	  WiseChip
		Show_String(1,&Name,0x08,XLevel+0x2A);
		sleep(3);

		Fill_RAM(0x00);			// Clear Screen
		
		//Show String +886-37-587168
		Show_String(1,&Tel_1,0x08,XLevel+0x20);
		Show_String(1,&Tel_2,0x09,XLevel+0x20);
